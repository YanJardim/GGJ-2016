        ...
        for (int i = 0; i < gridX; i++)
        {
            for (int j = 0; j < gridY; j++)
            {
                if (grid[i, j] == blocks.GRASS)
                {
                    ...
                    GameObject aux = Instantiate(grass, new Vector3(transform.position.x + i, transform.position.y + j, 0), grass.transform.rotation) as GameObject;
                    aux.transform.SetParent(GameObject.Find("Grid").transform);
                }
            }
        }
        GameObject.Find("Grid").transform.position = new Vector3(-16, -16, 0);
    }
}
